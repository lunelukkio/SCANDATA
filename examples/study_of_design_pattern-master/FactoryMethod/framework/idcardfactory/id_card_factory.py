...
class IDCardFactoryB(Factory):
    def __init__(self):
        self.owners = []

    def createProduct(self, owner):
        return IDCardProductB(owner)
...
class IDCardProductB(Product):
    def __init__(self, owner):
        self.owner = owner
        print("I'll create {0}'s card, type B".format(self.owner))

    def use(self):
        print("I'll use {0}'s card, type B".format(self.owner))
